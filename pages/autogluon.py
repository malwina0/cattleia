import dash
from dash import html, dcc, callback, Input, Output, State
import base64
import io
import dash_bootstrap_components as dbc
import sys
import metrics
import pandas as pd
import shutil
from autogluon.tabular import TabularPredictor
import zipfile
sys.path.append("..")


dash.register_page(__name__)

# wygląd strony
layout = html.Div([
    dcc.Store(id='csv_data_ag', data=[], storage_type='memory'),
    dcc.Store(id='y_label_column_ag', data=[], storage_type='memory'),
    dbc.Row([
        dbc.Col([
            dbc.Container([
                html.H5("Upload csv data", className="sidepanel_text"),
                dcc.Upload(
                    id='upload_csv_data_ag',
                    children=html.Div([
                        'Drag and Drop or ',
                        html.A('Select Files')
                    ]),
                    className="upload_data",
                    multiple=True
                ),
                html.Div(id='select_y_label_column_ag'),
                html.Div(id='upload_model_section_ag'),
            ], className="px-3 sidepanel")
        ], width=2),
        dbc.Col([
            dcc.Loading(id="loading-1_ag", type="default", children=html.Div(id="plots_ag"), className="spinxD")
        ], width=10)
    ])
])


# funkcja odpowiedzialna za prawidlowe wczytanie danych
def parse_data(contents, filename):
    content_type, content_string = contents.split(",")

    decoded = base64.b64decode(content_string)
    try:
        if "csv" in filename:
            df = pd.read_csv(io.StringIO(decoded.decode("utf-8")))
        elif "pkl" in filename:
            df = pd.read_pickle(io.BytesIO(decoded))
        elif "txt" or "tsv" in filename:
            df = pd.read_csv(io.StringIO(decoded.decode("utf-8")), delimiter=r"\s+")
    except Exception as e:
        print(e)
        return html.Div(["There was an error processing this file."])

    return df


# czesc odpowiedzialna za dodanie pliku csv
@callback(
    [Output('csv_data_ag', 'data'),
        Output('select_y_label_column_ag', 'children')],
    [Input('upload_csv_data_ag', 'contents'),
        State('upload_csv_data_ag', 'filename')]
)
def update_output(contents, filename):
    data = []
    children = []
    if contents:
        contents = contents[0]
        filename = filename[0]
        df = parse_data(contents, filename)
        data = df.to_dict()

        children = html.Div([
            html.P(filename, className="sidepanel_text"),
            html.Hr(),
            html.H5("Select target colum", className="sidepanel_text"),
            dcc.Dropdown(id='column_select_ag',
                         options=[{'label': x, 'value': x} for x in df.columns]),
            html.Hr(),
        ])

    return data, children


# Wybranie kolumny
@callback(
    [Output('y_label_column_ag', 'data'),
        Output('upload_model_section_ag', 'children')],
    Input('column_select_ag', 'value')
)
def select_kolumns(value):
    children = html.Div([
        html.H5("Upload FLAML model", className="sidepanel_text"),
        dcc.Upload(
            id='upload_model_ag',
            children=html.Div([
                'Drag and Drop or ',
                html.A('Select Files')
            ]),
            className="upload_data",
            multiple=True
        ),
        html.Hr(),
    ])
    data = {'name': value}

    return data, children


# funkcja odpowiedzialna za prawidlowe wczytanie modelu z pliku zip
def parse_data_model(contents):
    if contents is not None:
        content_type, content_string = contents.split(',')
        decoded = base64.b64decode(content_string)

        # Rozpakowanie pliku ZIP
        with zipfile.ZipFile(io.BytesIO(decoded), 'r') as zip_ref:
            zip_ref.extractall('./uploaded_model')

        predictor = TabularPredictor.load('./uploaded_model')
    # Usunięcie wczytanego folderu modelu po wczytaniu

    return predictor


# wybranie modelu a nastepnie narysowanie wykresow
@callback(
    Output('plots_ag', 'children'),
    Input('upload_model_ag', 'contents'),
    State('upload_model_ag', 'filename'),
    State('csv_data_ag', 'data'),
    State('y_label_column_ag', 'data'),
)
def update_model(contents, filename, df, column):
    children = []
    if contents:
        contents = contents[0]

        # usunięcie wrazie gdyby był już ten folder
        try:
            shutil.rmtree('./uploaded_model')
        except FileNotFoundError:
            pass
        model = parse_data_model(contents)

        df = pd.DataFrame.from_dict(df)
        df = df.dropna()

        X = df.iloc[:, df.columns != column["name"]]
        y = df.iloc[:, df.columns == column["name"]]
        y = y.squeeze()

        # sprawdzenie czy klasyfikacja czy regresja
        if isinstance(y[0], (int, float)):
            task = "regression"
        else:
            task = "classification"

        if task == "regression":
            plot_component = [
                dbc.Row([
                    dbc.Col([dcc.Graph(figure=metrics.mse_plot(model, X, y, library="AutoGluon"), className="plot")], width=6),
                    dbc.Col([dcc.Graph(figure=metrics.mape_plot(model, X, y, library="AutoGluon"), className="plot")], width=6),
                ]),
                dcc.Graph(figure=metrics.mae_plot(model, X, y, library="AutoGluon"), className="plot"),
                dcc.Graph(figure=metrics.correlation_plot(model, X, library="AutoGluon", task=task), className="plot"),
                dcc.Graph(figure=metrics.prediction_compare_plot(model, X, y, library="AutoGluon", task=task), className="plot"),
            ]
        else:
            plot_component = [
                dbc.Row([
                    dbc.Col([dcc.Graph(figure=metrics.accuracy_plot(model, X, y, library="AutoGluon"), className="plot")], width=6),
                    dbc.Col([dcc.Graph(figure=metrics.precision_plot(model, X, y, library="AutoGluon"), className="plot")], width=6),
                ]),
                dbc.Row([
                    dbc.Col([dcc.Graph(figure=metrics.recall_plot(model, X, y, library="AutoGluon"), className="plot")], width=6),
                    dbc.Col([dcc.Graph(figure=metrics.f1_score_plot(model, X, y, library="AutoGluon"), className="plot")], width=6),
                ]),
                dcc.Graph(figure=metrics.correlation_plot(model, X, library="AutoGluon", task=task), className="plot"),
                dcc.Graph(figure=metrics.prediction_compare_plot(model, X, y, library="AutoGluon", task=task), className="plot"),
            ]

        for plot in metrics.permutation_feature_importance_all(model, X, y, library="AutoGluon", task=task):
            plot_component.append(dcc.Graph(figure=plot, className="plot"))

        for plot in metrics.partial_dependence_plots(model, X, library="AutoGluon", autogluon_task=task):
            plot_component.append(dcc.Graph(figure=plot, className="plot"))

        children = html.Div(plot_component)
        shutil.rmtree('./uploaded_model')
    return children
